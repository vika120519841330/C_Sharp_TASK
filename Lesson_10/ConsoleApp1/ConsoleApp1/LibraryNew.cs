/*Создать коллекцию элементов, ключами которой будут выступать даты последнего посещения библиотеки, а значениями –
экземпляры класса Person, созданного ранее. Класс Person также требуется видоизменить: массив книг (строк)
должен быть преобразован в коллекцию, каждый элемент которой хранит название, автора и год издания. Расширение может быть
выполнено за счет создания дополнительного класса Book.*/
using System;
